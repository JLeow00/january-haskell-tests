...
execStatement e'@(DoWhile b e) s
  | eval e s' == 1 = execStatement e' s'
  | otherwise      = s'
  where
    s' = execBlock b s
...
foldConst (Apply o (Const c) (Const c'))
  = Const (apply o c c')
foldConst (Apply Add (Var i) (Const 0))
  = Var i
foldConst (Apply Add (Const 0) (Var i))
  = Var i
foldConst e
  = e
...
sub i v (Var i')
  | i == i' = Const v
sub i v (Apply o e e')
  = foldConst (Apply o (sub i v e) (sub i v e'))
sub i v (Phi e e')
...
scan i v []
  = ([], [])
scan i v (s@(Assign i' (Const c)) : b)
  | i' /= "$return" = ((i', c) : w, b')
  | otherwise       = (w, s : b')
  where
    (w, b') = scan i v b
scan i v (s@(Assign i' e) : b)
  | s /= s'   = scan i v (s' : b)
  | otherwise = (w, s : b')
  where
    s'      = Assign i' (sub i v e)
    (w, b') = scan i v b
scan i v (If e b1 b2 : b)
  = (w1 ++ w2 ++ w, If (sub i v e) b1' b2' : b')
  where
    (w1, b1') = scan i v b1
    (w2, b2') = scan i v b2
    (w, b')   = scan i v b
scan i v (DoWhile b1 e : b)
  = (w1 ++ w, DoWhile b1' (sub i v e) : b')
  where
    (w1, b1') = scan i v b1
    (w, b')   = scan i v b
...
propagateConstants 
  ...
  where
    ...
    propagateConstants' [] b
      = b
    propagateConstants' ((i, v) : w) b
      -- | trace ("PropConstants' " ++ show ((i, v):w)) False = undefined
...
unPhi :: Block -> Block
-- Pre: the block is in SSA form
unPhi (If e b1 b2 : Assign i (Phi e1 e2) : b)
  = unPhi (If e b1' b2' : b)
  where
    b1' = unPhi b1 ++ [Assign i e1]
    b2' = unPhi b2 ++ [Assign i e2]
unPhi (DoWhile (Assign i (Phi e1 e2) : b) e : b')
  = Assign i e1 : unPhi (DoWhile (b ++ [Assign i e2]) e : b')
unPhi (If e b1 b2 : b)
  = If e (unPhi b1) (unPhi b2) : unPhi b
unPhi (DoWhile b1 e : b)
  = DoWhile (unPhi b1) e : unPhi b
unPhi (s : b)
  = s : unPhi b
unPhi []
  = []
